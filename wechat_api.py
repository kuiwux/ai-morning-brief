...
def _get_env_dir() -> str:
    """返回 .env 文件所在目录（项目根或 pipeline/）"""
    # 先检查当前目录（项目根）
    root_env = os.path.join(WORKDIR, ".env")
    if os.path.exists(root_env):
        return WORKDIR
    # 回退到 pipeline/ 子目录
    pipeline_env = os.path.join(WORKDIR, "pipeline", ".env")
    if os.path.exists(pipeline_env):
        return os.path.join(WORKDIR, "pipeline")
    # 最后返回 WORKDIR（让后续报错处理）
    return WORKDIR
